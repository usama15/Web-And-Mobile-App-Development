...
async function info(...args) {
    ...
    const parsedArgs = cli.argParser(options_1.default, args, true);
    const infoArgs = parsedArgs.opts;
    const envinfoConfig = {};
    if (parsedArgs.unknownArgs.length > 0) {
        logger_1.default.error(`Unknown argument: ${parsedArgs.unknownArgs}`);
        process.exit(2);
    }
    if (infoArgs.output) {
        ...
        switch (output) {
            ...
            case 'json':
                ...
                break;
            default:
                logger_1.default.error(`${infoArgs.output} is not a valid value for output\n`);
        }
    }
    ...
    logger_1.default.raw(finalOutput);
    return finalOutput;
}
